...
[NOTE]
All instructions in this section assume your console's current working directory is the location where the scripts are found.
...
or you can run the `deploy-agent.sh` script. The instructions you see below are essentially what the `deploy-agent.sh` script does.
