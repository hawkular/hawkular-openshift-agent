...
2) Once OpenShift Origin is fully started and you can log into the console (at `http://${OPENSHIFT_IP_ADDRESS}:8443`), you need to complete its setup by running the "after-start" script. Run this in another console since the `start-openshift.sh` you ran in step 1 runs in the foreground:
