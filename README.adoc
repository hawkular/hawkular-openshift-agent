...
- name: my.domain:type=MyApp,component=*#Count
  id: Count of my bean ${component}
```

Note that in your metric ID you can use ${x} tokens where the "x" is the MBean name key that was set to the "*" wildcard in the name. You can have multiple wildcards in your metric name such as:

```
metrics:
- name: my.domain:type=MyApp,component=*,subcomponent=*#Count
  id: Count of the ${subcomponent} of bean ${component}
```
...
Note that when the attribute is a wildcard, you can use the ${1} token in the ID where you want the name of the attribute in the metric ID.
...
- name: java.lang:type=Memory#HeapMemoryUsage#*
  id: Heap Memory Usage ${2}
```

Note that when the inner path of a composite attribute is a wildcard, you can use the ${2} token in the ID where you want the name of the inner path of the composite attribute in the metric ID.
